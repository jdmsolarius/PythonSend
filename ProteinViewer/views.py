...
tkinter.Image.__del__ = do_nothing
tkinter.Image.__init__ = do_nothing
tkinter.Image.__getattribute__ = do_nothing
...
def make_and_save_plot(pos_to_val, out_file="plot.png"):
    ...
    pyplot.tight_layout()
    pyplot.savefig(out_file, dpi=280, bbox_inches='tight')  
    img_io = BytesIO()
...
def fetch_pdb(proteinName):
    try:
        ...
        try:
            ...
            if protein_id != uniprot_Id:
                return fetch_pdb(protein_id)
        except:
...
def Get_PDB(Uniprot_Id):
    try:
        pos_to_vals = Database.get_data_tuples(Uniprot_Id)
        intermediate_image = gen_image(pos_to_vals)
        pdb_data = create_pathogenic_pdb(intermediate_image, Uniprot_Id)
        return pdb_data
    except:
        return None
...
def protein_name_map():
    data =  Database.get_protein_name_map()
    return jsonify(data)
